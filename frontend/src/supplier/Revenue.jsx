...
${index + 1}. Order #${order._id}
   ...
   Product: ${order.productId?.name || order.productName}
   Quantity: ${order.amount || 1}
   Unit Price: $${(order.productId?.price || 0).toFixed(2)}
   Payment Method: ${order.paymentMethod || 'Cash on Delivery'}
   Total Amount: $${((order.amount || 1) * (order.productId?.price || 0)).toFixed(2)}
   Your Revenue: $${revenue.toFixed(2)}
   Delivered: ${order.updatedAt ? new Date(order.updatedAt).toLocaleDateString() : 'N/A'}
...
REPORT END
Generated by FIX MATE Supplier Dashboard
`;
  ...
  const downloadReport = (content) => {
    ...
    link.href = url;
    link.download = `supplier-revenue-report-${supplierData.name}-${new Date().toISOString().split('T')[0]}.txt`;
              ...
              <div key={order._id} className="border border-gray-100 rounded-lg p-4 hover:bg-gray-50 transition">
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    <div className="font-bold text-green-600">+${(order.supplierRevenue || 0).toFixed(2)}</div>
