...
Hourly Rate: $${vendorData.hourlyRate}
...
${index + 1}. Order #${order._id}
   ...
   Payment Method: ${order.paymentMethod || 'Pay on Arrival'}
   Hours Worked: ${order.hoursWorked || 1}
   Total Amount: $${(order.totalAmount || vendorData.hourlyRate || 0).toFixed(2)}
   Your Revenue: $${revenue.toFixed(2)}
   Completed: ${order.updatedAt ? new Date(order.updatedAt).toLocaleDateString() : 'N/A'}
...
REPORT END
Generated by FIX MATE Vendor Dashboard
`;
  ...
  const downloadReport = (content) => {
    ...
    link.href = url;
    link.download = `revenue-report-${vendorData.name}-${new Date().toISOString().split('T')[0]}.txt`;
        ...
        <div className="bg-green-50 border border-green-200 rounded-xl p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-green-800">${revenueData.totalRevenue.toFixed(2)}</p>
        ...
        <div className="bg-orange-50 border border-orange-200 rounded-xl p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-orange-800">${revenueData.totalServiceFees.toFixed(2)}</p>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-8">
        ...
        <div className="space-y-3">
          <div className="flex justify-between items-center py-2 border-b border-gray-100">
            ...
            <span className="font-semibold">${(revenueData.totalRevenue + revenueData.totalServiceFees).toFixed(2)}</span>
          </div>
          <div className="flex justify-between items-center py-2 border-b border-gray-100">
            <span className="text-green-600">Your Share (80%)</span>
            <span className="font-semibold text-green-600">${revenueData.totalRevenue.toFixed(2)}</span>
          </div>
          <div className="flex justify-between items-center py-2">
            <span className="text-orange-600">Platform Fee (20%)</span>
            <span className="font-semibold text-orange-600">${revenueData.totalServiceFees.toFixed(2)}</span>
          </div>
        </div>
      </div>
              ...
              <div key={order._id} className="border border-gray-100 rounded-lg p-4 hover:bg-gray-50 transition">
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right">
                    <div className="font-bold text-green-600">+${(order.vendorRevenue || 0).toFixed(2)}</div>
